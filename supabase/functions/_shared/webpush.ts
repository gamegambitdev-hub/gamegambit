...
// Web Push encryption and VAPID JWT signing.
// Import with: import { sendWebPush } from "../_shared/webpush.ts"
...
export async function sendWebPush(
    ...
    try {
        ...
        console.warn("[webpush] sendWebPush error:", e);
    }
}
...
async function encryptWebPushPayload(
    ...
    const receiverPubKey = await crypto.subtle.importKey(
        "raw", receiverPubKeyBytes, { name: "ECDH", namedCurve: "P-256" }, true, []
    );
    ...
    const prkBits     = await crypto.subtle.deriveBits(
        { name: "HKDF", hash: "SHA-256", salt: authSecret, info: authInfo }, prkMaterial, 256
    );
    const prk = new Uint8Array(prkBits);

    const prkKey  = await crypto.subtle.importKey("raw", prk, "HKDF", false, ["deriveBits"]);
    const cekBits = await crypto.subtle.deriveBits(
        { name: "HKDF", hash: "SHA-256", salt, info: encoder.encode("Content-Encoding: aes128gcm\0") }, prkKey, 128
    );
    const nonceBits = await crypto.subtle.deriveBits(
        { name: "HKDF", hash: "SHA-256", salt, info: encoder.encode("Content-Encoding: nonce\0") }, prkKey, 96
    );
